...
| **AuditLine** | A single structured JSON log line emitted at `INFO` level by the `log_request` middleware at the end of every meaningful request. For `tools/call` requests it carries `upn`, `tool`, `bytes` (response JSON size), `http_code`, `run_msec` (end-to-end). For auth-flow events it carries `http_code`, `run_msec`, and a `message` string. Requests with no attached `HandlerLog` (initialize, ping, tools/list) are logged at `DEBUG` only. |
| **HandlerLog** | An Axum response extension (`pub(crate) struct HandlerLog`) set by a handler to pass structured context up to `log_request`. Use `HandlerLog::audit(upn, tool, bytes)` for `tools/call` and `HandlerLog::msg(text)` for auth events. Its presence is the gate: `log_request` emits INFO if present, DEBUG otherwise. |
| **CallerIdentity** | The authenticated user's identity extracted from the validated Entra JWT — at minimum UPN (`upn`/`preferred_username`) and OID (`oid`); used for audit logging and future authorization checks. Injected into Axum request extensions by `require_auth` middleware so downstream handlers can read it without re-validating the token. |
