...
| **AuditLine** | A single structured JSON log line emitted at `INFO` level at the end of every authenticated `tools/call` request — fields: `upn`, `tool`, `bytes` (response JSON size), `run_msec`. Written via `tracing::info!` so it carries the same timestamp/level envelope as all other log output. |
